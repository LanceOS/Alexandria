...
The terminal example selects C++20 and explicitly assumes an installed GCC C++ compiler and standard library on Linux or macOS. It is not a universal compiler setup command. Alexandria's optional [code area](code-runner.md) compiles and runs C++20 in isolated containers on the local server, with input and output alongside the lesson.
