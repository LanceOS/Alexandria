...
`content:import -- cpp` imports one root after a verified backup, using a single transaction. It adds new units, modules, and explicit later releases, leaves exact matching records unchanged, and rejects conflicts without replacing published work. The compatibility command `content:cpp-basics` imports the C++ root through the same path. Module JSON accepts a positive `version` (default 1); a correction uses the next release number and a new `versionId` while preserving stable module and part IDs. Unchanged source metadata may be reused; bibliographic corrections require new source IDs. Earlier releases and learner history remain intact. Fresh installations may begin at the current selected release; existing installations reject skipped or older releases. See [Content authoring](../content/README.md) and [C++ Basics content](cpp-basics-content.md).
