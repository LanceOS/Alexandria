...
2. Add one JSON file per module, using an existing module as a format reference. Assign new IDs to the module, first release, lesson parts, and citations. Follow the [curriculum depth standard](../notes/curriculum-depth.md) and [evidence policy](../docs/curriculum-evidence.md): teach the objectives with detailed explanations and worked examples, verify technical claims against primary documentation, and use relevant peer-reviewed research for research-based claims. Include precise locators and record the inspected evidence.
...
All included modules use exactly three ordered parts and follow the standing [curriculum depth standard](../notes/curriculum-depth.md). Every module needs measurable objectives and prerequisites, detailed conceptual explanations, a step-by-step worked example, a second contrasting or boundary case, at least two meaningful practice tasks with explained answers, relevant pitfalls and limits, a recap, and sources. Distribute teaching across all three parts. Section titles should describe the lesson's idea rather than repeat a generic template heading. New subjects state prerequisites and their boundary with C++ in the lesson or unit introduction. Language-neutral traces use `text` blocks and clearly state their assumptions; they are not presented as executable programs. Fast learning comes from clear organization and sequencing; do not impose a short-description or fixed-duration limit on a lesson.
