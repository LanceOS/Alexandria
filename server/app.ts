...
import { createCodeRunner } from './services/code-runner.js';
...
export async function buildApp(config: Config, options: AppOptions = {}) {
  ...
  try {
    ...
    registerProgressRoutes(app, auth, connection);
    registerPracticeRoutes(app, connection);
    registerCodeRunnerRoutes(app, auth, createCodeRunner(config.codeRunner));
